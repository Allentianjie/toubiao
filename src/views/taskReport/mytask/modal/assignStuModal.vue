<template>
  <div id="preView" style="width:100%">
    <Modal id="preViewBox" class-name="vertical-center-modal" v-model="assignmentData.state" width="50%"
           :mask-closable="false"
  ...
  export default {
    ...
    methods: {
            ...
            this.$confirm("请确认操作？", "提示", {
              ...
              .then(() => {
                ...
                if (that.targetForm[that.itemIndex].dataType == '2') {
                  form.targetValue = (form.targetValue / 100).toFixed(2);
                  form.warnValue = (form.warnValue / 100).toFixed(2);
                } else if (that.targetForm[that.itemIndex].dataType == '3') {
                  form.targetValue = (form.targetValue / 1000).toFixed(2);
                  form.warnValue = (form.warnValue / 1000).toFixed(2);
                } else {
                }
                let formData = {
      ...
      removeAllocateTaskItem(row, ind) {
        ...
        if (Number.isInteger(taskId)) {
          this.$confirm(
            "执行该操作将同时删除下级所有已经分派并执行的子诊改，确认删除吗？",
            "提示",
            {
              type: "warning"
            }
      ...
      changeVlue(val) {
        //编辑
        if (val == '2') {
          this.assignmentData.data.targetValue = (this.assignmentData.data.targetValue * 100).toFixed(2);
          this.assignmentData.data.warnValue = (this.assignmentData.data.warnValue * 100).toFixed(2);
        } else if (val == '3') {
          this.assignmentData.data.targetValue = (this.assignmentData.data.targetValue * 1000).toFixed(2);
          this.assignmentData.data.warnValue = (this.assignmentData.data.warnValue * 1000).toFixed(2);
        }
