  ...
  export default {
    ...
    methods: {
      ...
      roleUserSearch(pageindex, pagesize) {
        ...
        NeizhenUtil.isEmpty(this.searchName.searchRoleUser)
          ? {}
          : (queryobj.searchuser = this.searchName.searchRoleUser);
        NeizhenUtil.isEmpty(this.roleSelect)
          ? {}
          : (queryobj.roleId = this.roleSelect.id);
        this.$AsyncDealData(
          datarouters.SystemManage.user.getUsersByrole,
          queryobj,
          function(data) {
            that.roleData.data = data.data.records;
            that.roleData.total = data.data.total;
            setTimeout(() => {
              that.setCheckedTwo()
            }, 500)
          },
