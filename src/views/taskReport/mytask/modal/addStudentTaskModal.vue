<template>
  <div id="preView" style="width:100%">
    <Modal id="preViewBox" class-name="vertical-center-modal" v-model="addStudentData.state" width="50%"
           :mask-closable="false"
    ...
    >
      ...
      <Form :label-width="100">
        <div class="fileManage">
            ...
            >
              <Row>
                <Col span="12">
                  ...
                  <FormItem v-if="modalType=='2'" label="学生" prop="directorVos">
                    <Button
                  ...
                  <FormItem v-if="modalType=='1'" label="教职工" prop="directorVos">
                    <Button
                      @click="openPrincipals(taskForm,taskIndex)"
                      class="add-doc"
                      ghost
                      type="success"
                      size="small"
          ...
          <Principal
            ...
            @closePrincipals="closePrincipals"
          ></Principal>
          <Principals
            ref="principals"
            :PrincipalData="PrincipalDatas"
      ...
      <div slot="footer" style="text-align: center">
        ...
        <Button type="warning" size="large" @click="closeAssignmentModal2">关闭</Button>
      </div>
    </Modal>
  </div>
</template>
...
<script>
  import Principals from "@/components/teacher";
  ...
  export default {
    ...
    components: {AddFileModal, Principals, Principal, BasicData, Preview},
    data() {
      return {
        modalType: "",
        noData: true,
    ...
    computed: {
      hierarchyName: function () {
        let task = this.addStudentData.data;
        return (
          task.hieName +
          " / " +
          task.levelName +
    ...
    methods: {
      ...
      changeDeptdata(index, val) {
        ...
        this.options4 = [];
        if (val != undefined) {
          this.$Get('admin/database/getByIdentIdAndDeptId?identId=' + this.addStudentData.data.identId + '&deptId=' + val, null, (res) => {
            if (res.data != null) {
      ...
      searchData() {
        this.options4 = [];
        let params = {
          deptId: this.addStudentData.deptId,
          parentId: this.addStudentData.dataId,
          dataName: this.targetForm[this.itemIndex].dataName, // 名称
          layType: this.addStudentData.hieId
        };
        fetchValueLists(
          Object.assign(
            {
      ...
      //获取部门
      getDeptData() {
        this.$Get("/admin/dept/getByPid?parentId=" + this.addStudentData.data.deptId, null, res => {
            // console.log(res)
      ...
      //获取当前任务的分派任务
      getAllocateTaskss(val) {
        // console.log(this.addStudentData);
        this.modalType = val;
        // this.noData = true;
        this.getDeptData();
        let startTime = this.addStudentData.data.startTime;
        let endTime = this.addStudentData.data.endTime;
        this.modalParam.minTime = new Date(startTime.replace("-", "/"));
        this.modalParam.maxTime = new Date(endTime.replace("-", "/"));
        // let allocateTasks = this.addStudentData.data.allocateTasks;
        // //清空分派集合
        ...
        //       dataId: task.dataId,
        //       dataType: (this.addStudentData.data.dataType).toString(),
        //       taskId: task.taskId,
          ...
          if (valid) {
            this.$Modal.confirm({
              ...
              onOk: () => {
                ...
                let formData = {
                  dataParentId: that.addStudentData.data.dataId,
                  ...
                  parentId: that.addStudentData.data.taskId,
                ...
                that.$Post("admin/task/student", formData, res => {
                  // console.log(res);
                  if (res.code == 0) {
      ...
      additem() {
        ...
        if (this.noData == false) {
          ...
          this.changeVlue(this.addStudentData.data.dataType);
          // console.log(this.addStudentData.data.dataType)
          this.targetForm.push({
            deptId: "",
            dataType: (this.addStudentData.data.dataType).toString(),
            taskName: "",
            ...
            dataName: "",
            dataId: this.addStudentData.data.dataId,
            targetStrategy: this.addStudentData.data.targetStrategy,
            targetValue: (this.addStudentData.data.targetValue).toString(),
            standardStrategy: this.addStudentData.data.standardStrategy,
            standardValue: (this.addStudentData.data.standardValue).toString(),
            standardWeight: this.addStudentData.data.standardWeight,
            targetWeight: this.addStudentData.data.targetWeight,
            warnStrategy: this.addStudentData.data.warnStrategy,
            warnValue: (this.addStudentData.data.warnValue).toString(),
            disValue: this.addStudentData.data.disValue,
            dataValue: this.addStudentData.data.dataValue,
            directorVos: [],
            targetFileVos: [],
            unEndTask: true,
            updateModal: false
          });
          this.targetForm[0].dataName = this.addStudentData.data.taskName + this.addStudentData.data.dataName;
          this.targetForm[0].taskName = this.addStudentData.data.targetName
      ...
      changeVlue(val) {
        //编辑
        if (val == '2') {
          this.addStudentData.data.targetValue = this.addStudentData.data.targetValue * 100;
          this.addStudentData.data.warnValue = this.addStudentData.data.warnValue * 100;
          this.addStudentData.data.standardValue = this.addStudentData.data.standardValue * 100;
        } else if (val == '3') {
          this.addStudentData.data.targetValue = this.addStudentData.data.targetValue * 1000;
          this.addStudentData.data.warnValue = this.addStudentData.data.warnValue * 100;
          this.addStudentData.data.standardValue = this.addStudentData.data.standardValue * 1000;
        }
      ...
      openBasicData(row, ind) {
        this.basicData.identId = this.addStudentData.data.identId; //最外层传的id
      ...
      //关闭老师负责人
      closePrincipal(data) {
        this.PrincipalDatas.state = false;
        if (data && data.length > 0) {
          data.forEach(o => {
            this.targetForm[this.PrincipalIndex].directorVos.push({
              userId: o.userId,
      ...
      openPrincipal(row, ind) {
        ...
        this.PrincipalData.data = this.addStudentData;
        this.PrincipalData.taskPid = this.addStudentData.taskPid
        this.$refs.principal.loadData();
      },
      //打开教职工负责人
      openPrincipals(row, ind) {
        this.PrincipalIndex = ind; //保存点击负责人的下标，方便上面 closePrincipal方法里返回数据赋值====this.targetForm[this.PrincipalIndex]
        this.PrincipalDatas.data = this.addStudentData;
        this.PrincipalDatas.state = true;
        this.PrincipalDatas.taskPid = this.addStudentData.taskPid
        this.$refs.principals.loadData();
      },
      //删除负责人
