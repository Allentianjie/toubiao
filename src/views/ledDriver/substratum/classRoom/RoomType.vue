...
<template>
  <div class="left-body">
    <div class="head">
      <span><i class="el-icon-location" style="color: #129B7F; padding-right: 8px;"></i>{{name}}</span>
      <button type="primary" ghost @click="$router.back(-1)">返回上级</button>
    </div>
    <div class="main">
      <div class="layer">
        <div class="layer-body">
          <div class="title">下级间数</div>
          <div class="con">
...
<script>
  ...
  export default {
    ...
    methods: {
      // 初始化
      initFn(strToObj) {
        this.$Get("/admin/subDrillClasses/getClassesTypeList?correId=" + strToObj.correId+"&field3="+ strToObj.dataId, null, res => {
          let barArry = res.data;
          let bar_xAxis = [];
          let bar_seriesData = [];
          for (let i = 0; i < barArry.length; i++) {
            bar_xAxis.push(barArry[i].field5);
