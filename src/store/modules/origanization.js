...
import {INPUT_DATA,UPDATED_ON } from '@/assets/data/data'
     ...
     state :{
      ...
      mutations : {
        ...
        createTree(state, datas) {
          ...
          state.chart = setTreeData(datas,'id', 'parentId', 'children')[0]
          // state.chart = createTree(datas)[0]
          state.chart.showChildren = true
          console.log(7777,state.chart)
        },
        ...
        addLine(state) {
          ...
          console.log(111,state.lines)
        },
        removeLines(state) {
          state.lines = []
          console.log(222,state.lines)
        },
...
function setTreeData(source, id, parentId, children) {
  ...
  const tree = cloneData.filter(father => {
    ...
    if (branchArr.length > 0) {
      ...
      // let copyFather=JSON.parse(JSON.stringify(father))
      let copyFather=father
      father[children].forEach((item,index)=>{
        // father[children][index].showChildren=true
        father[children][index].parent=copyFather.id?copyFather:null
    })
    } else {
      father[children] = []
    }
...
function createTree(array, parent, nextparent, tree) {
  ...
  if (!parent.id) {
    ...
    // let copyNextParent=JSON.parse(JSON.stringify(nextparent))
    let copyNextParent=nextparent
    parent['parent'] = copyNextParent.id ? copyNextParent : null
  }
  if (children.length) {
