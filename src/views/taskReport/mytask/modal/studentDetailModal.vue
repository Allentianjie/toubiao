<template>
  <div id="preView" style="width:100%">
    <Modal id="preViewBox" class-name="vertical-center-modal" v-model="studentDetailData.state" width="50%"
           :mask-closable="false"
           @on-cancel="closeAssignmentModal4"
    >
                ...
                <Col span="12" style="padding-left: 30px">
                  ...
                  <FormItem label="当前值">
                    <Row v-if="taskForm.dataType !== 5">
                      ...
                      <!--<Col :span="12">-->
                      <!--&nbsp; &nbsp;-->
                      <!--<span>{{taskForm.disValue | fullstandardValue(taskForm.dataType)}}</span>-->
                      <!--</Col>-->
                    </Row>
                    <Row v-else>
      ...
      <div slot="footer" style="text-align: center">
        ...
        <Button type="warning" size="large" @click="closeAssignmentModal4('targetForm')">关闭</Button>
      </div>
    </Modal>
  </div>
</template>
...
<script>
  import Principal from "@/components/student";
  ...
  export default {
    ...
    computed: {
      hierarchyName: function () {
        let task = this.studentDetailData.data;
        return (
          task.hieName +
          " / " +
          task.levelName +
    ...
    methods: {
      ...
      changeDeptdata(index, val) {
        ...
        this.options4 = [];
        if (val != undefined) {
          this.$Get('admin/database/getByIdentIdAndDeptId?identId=' + this.studentDetailData.data.identId + '&deptId=' + val, null, (res) => {
            if (res.data != null) {
      ...
      searchData() {
        this.options4 = [];
        let params = {
          deptId: this.studentDetailData.deptId,
          parentId: this.studentDetailData.dataId,
          dataName: this.targetForm[this.itemIndex].dataName, // 名称
          layType: this.studentDetailData.hieId
        };
        fetchValueLists(
          Object.assign(
            {
      ...
      //获取部门
      getDeptData() {
        this.$Get("/admin/dept/getByPid?parentId=" + this.studentDetailData.data.deptId, null, res => {
      ...
      getAllDetail() {
        this.addDataState=false;
        this.openIndex=this.studentDetailData.openIndex;
        this.noData = true;
        this.getDeptData();
        let startTime = this.studentDetailData.startTime;
        let endTime = this.studentDetailData.endTime;
        this.modalParam.minTime = new Date(startTime.replace("-", "/"));
        this.modalParam.maxTime = new Date(endTime.replace("-", "/"));
        let allocateTasks = [];
        allocateTasks.push(this.studentDetailData.data);
        //清空分派集合
        ...
        if (allocateTasks.length > 0) {
          allocateTasks.forEach(task => {
            if (this.studentDetailData.data.dataType == 2) {
              task.warnValue = (task.warnValue * 100).toString();
              task.targetValue = (task.targetValue * 100).toString();
              task.standardValue = (task.standardValue * 100).toString()
            }
            if (this.studentDetailData.data.dataType == 3) {
              task.warnValue = (task.warnValue * 1000).toString();
              task.targetValue = (task.targetValue * 1000).toString();
              task.standardValue = (task.standardValue * 1000).toString()
            }
            // console.log(allocateTasks)
            this.targetForm.push({
              deptId: this.studentDetailData.deptId ,
              dataId: task.dataId,
              dataType: (this.studentDetailData.data.dataType).toString() ,
              taskId: task.taskId,
              ...
              standardValue: task.standardValue ,
            ...
            this.$Modal.confirm({
              ...
              onOk: () => {
                ...
                if (form.startTime.valueOf() < that.modalParam.minTime.valueOf()) {
                  that.$Message.warning("分派任务的开始时间不能早于当前任务的开始时间");
                  return;
                }
                ...
                if (form.endTime.valueOf() > that.modalParam.maxTime.valueOf()) {
                  that.$Message.warning("分派任务的结束时间不能晚于当前任务的结束时间");
                  return;
                }
                ...
                let formData = {
                  ...
                  dataId: form.dataId,
                  parentId: that.studentDetailData.data.taskId,
                ...
                if (that.addDataState == false) {
                  ...
                  if (Number.isInteger(taskId)) {
                    ...
                    if(that.openIndex==1){
                      that.$Put("/admin/task", param, function (data) {
                        ...
                        that.$emit("closeAssignmentModal4", {
                          state: false,
                          reload: that.reloadTaskTable
                        });
                      });
                    }else if(that.openIndex==2){
                      that.$Put("/admin/task/spiral", param, function(data) {
                        // console.info(data);
                        that.$Message.success("成功诊改分派任务");
                        that.$emit("closeAssignmentModal4", {
                          state: false,
                          reload: that.reloadTaskTable
                        });
                  ...
                  let params = {
                    ...
                    parentId: that.studentDetailData.dataId,
                      ...
                      if (Number.isInteger(taskId)) {
                        ...
                        if(that.openIndex==1){
                          that.$Put("/admin/task", param, function (data) {
                            ...
                            that.studentDetailData.state=false
                          });
                        }else if(that.openIndex==2){
                          that.$Put("/admin/task/spiral", param, function(data) {
                            // console.info(data);
      ...
      additem() {
        ...
        if (this.noData == false) {
          ...
          this.changeVlue(this.studentDetailData.data.dataType);
          ...
          this.targetForm.push({
            ...
            dataName: "",
            dataId: this.studentDetailData.data.dataId,
            targetStrategy: this.studentDetailData.data.targetStrategy,
            targetValue: this.studentDetailData.data.targetValue,
            standardStrategy: this.studentDetailData.data.standardStrategy,
            standardValue: this.studentDetailData.data.standardValue,
            standardWeight: this.studentDetailData.data.standardWeight,
            targetWeight: this.studentDetailData.data.targetWeight,
            warnStrategy: this.studentDetailData.data.warnStrategy,
            warnValue: this.studentDetailData.data.warnValue,
            disValue: this.studentDetailData.data.disValue,
            dataValue: this.studentDetailData.data.dataValue,
            directorVos: [],
            targetFileVos: [],
            unEndTask: true,
            updateModal: false
      ...
      changeVlue(val) {
        //编辑
        if (val == '2') {
          this.studentDetailData.data.targetValue = (this.studentDetailData.data.targetValue * 100).toFixed(2);
          this.studentDetailData.data.warnValue = (this.studentDetailData.data.warnValue * 100).toFixed(2);
          this.studentDetailData.data.standardValue = (this.studentDetailData.data.standardValue * 100).toFixed(2);
        } else if (val == '3') {
          this.studentDetailData.data.targetValue = (this.studentDetailData.data.targetValue * 1000).toFixed(2);
          this.studentDetailData.data.warnValue = (this.studentDetailData.data.warnValue * 1000).toFixed(2);
          this.studentDetailData.data.standardValue = (this.studentDetailData.data.standardValue * 1000).toFixed(2);
        }
      ...
      openBasicData(row, ind) {
        this.basicData.identId = this.studentDetailData.data.identId; //最外层传的id
      ...
      openPrincipal(row, ind) {
        ...
        this.PrincipalData.data = this.studentDetailData;
      ...
      closeAssignmentModal4() {
        this.$emit("closeAssignmentModal4", {
          state: false,
        });
      },
      //获取策略符号
