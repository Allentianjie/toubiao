<template>
  <div id="majorBarTwoChar" style="width:100%;height:100%" :class="nodata">
...
<script>
  ...
  export default {
    ...
    methods: {
      barChart() {
        let that = this
        let {id, obj, seriesData,} = this.barChartData;
        // console.log(555,seriesData)
        let doc = document.querySelector("#majorBarTwoChar");
          ...
          for(let i = 0; i<seriesData.yData.length; i++){
            if(seriesData.yData[i] > 0){
              ...
              myChart.setOption(
                {
                  ...
                  tooltip : {
                    ...
                    formatter: function (params) {
                      var tar = params[0];
                      return tar.name + '<br/>' + tar.seriesName +' : ' + tar.value +' '+ obj[tar.dataIndex].amoutUnit;
                    }
                  },
                  toolbox: {
                    feature: {
              ...
              myChart.on('click', function (params) {
                ...
                that.$router.push({path: "/propertyTable", query: {obj: params.name}});
                /*seriesData.xData.map((item,index)=>{
                  if(item === params.name){
                    let seriseId = seriesData.id[index]
                    let obj = {
                      seriseId: seriseId,
                      type:seriesData.type.type,
                      titleName:seriesData.type,
                    }
                    that.$router.push({path: "/majorThreeTable", query: {obj: obj}});
                  }
                })*/
              })
              this.$nextTick(() => {
