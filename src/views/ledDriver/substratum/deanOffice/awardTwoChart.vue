...
export default {
  ...
  methods:{
    getData(obj){
      let that = this
     if(obj.symbol === '论文'){
       that.$Get("/admin/subDrillSchcool/getSerisePaperNumber", null, function (data) {
         // console.log('论文',data)
         that.getBarData(data.data.data,obj.symbol)
         that.getLineData(data.data.data2)
       })
     }else if(obj.symbol === '科研'){
       that.$Get("/admin/subDrillSchcool/getSeriseResearchPrizes", null, function (data) {
         // console.log( '科研',data)
         that.getBarData(data.data.data,obj.symbol)
         that.getLineData(data.data.data2)
       })
    ...
    // 柱状图
    getBarData(data,symbol){
      // console.log(33,data)
      // console.log(44,symbol)
      let that = this
      let xDataBar = []
      let yDataBar = []
      let idBar = []
