  ...
  export default {
    ...
    methods: {
      radarChart() {
        ...
        let {id, data} = this.comData;
        let doc = document.querySelector("#comChar");
        if (data.taskFinishies && data.taskFinishies.length > 0 && data.taskNames && data.taskNames.length > 0 && data.taskTotal && data.taskTotal.length > 0) {
          this.nodata = "";
